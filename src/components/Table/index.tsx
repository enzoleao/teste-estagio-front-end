...
import { Button, FormControl, FormControlLabel, FormLabel, Input, InputAdornment, InputLabel, Radio, RadioGroup, TextField } from '@mui/material'
import { RxMagnifyingGlass } from 'react-icons/rx'
import { useEffect, useMemo, useState } from 'react'

export function Table() {
  const { companies, pageToShowOnTable, setPageToShowOnTable, setCompanies, maxPage } = useAllContexts()
  ...
  const handleChangeSearchMode = (event: React.ChangeEvent<HTMLInputElement>) => {
          ...
          <div className={styles.tableContainer}>
            <header>
              <div>
                  ...
                  <Input
                    ...
                    endAdornment={
                      <InputAdornment position="end">
                        ...
                        className={styles.iconSearchInput} cursor="pointer" 
                        />
                      </InputAdornment>
                    }
                  />
                ...
                <FormControl>
                  <FormLabel id="demo-row-radio-buttons-group-label">Modo de pesquisa</FormLabel>
                  <RadioGroup
                    row
                  ...
                  >
                    <FormControlLabel value="Empresa" control={<Radio />} label="Nome" />
            ...
            (<footer>
              <button 
                disabled={pageToShowOnTable<= 1 ? true : false} 
                className={`${pageToShowOnTable<= 1 ?'cursor-not-allowed' : 'cursor-pointer' }`} 
                onClick={lastPage}>
                  {`<`}
              </button>
              <p>{pageToShowOnTable}</p>
              <button
               disabled={pageToShowOnTable === maxPage ? true : false} 
               className={`${pageToShowOnTable === maxPage ?'cursor-not-allowed' : 'cursor-pointer' }`} 
               onClick={nextPage}>
                {`>`}
              </button>
            </footer>)
